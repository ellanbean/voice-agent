...
Register a SIP outbound trunk with LiveKit from one of the sip/*.json files (no `lk` CLI needed).
  ...
  python sip_setup.py --delete ST_xxxx                    # remove a trunk
...
async def main():
    ...
    try:
        if "--delete" in sys.argv:
            tid = sys.argv[sys.argv.index("--delete") + 1]
            await lk.sip.delete_sip_trunk(api.DeleteSIPTrunkRequest(sip_trunk_id=tid)); print("deleted", tid); return
        if "--list" in sys.argv:
            res = await lk.sip.list_sip_outbound_trunk(api.ListSIPOutboundTrunkRequest())
            for t in res.items:
                print(f"{t.sip_trunk_id}  {t.name:<20} {t.address:<35} numbers={list(t.numbers)} country={t.destination_country or '-'}")
            return
        path = sys.argv[1]
        ...
        trunk = api.SIPOutboundTrunkInfo(
            ...
            # ISO country of the numbers this trunk dials: LiveKit then sends SIP+media from that region.
            # Required for India ("Domestic Anchored Terms Not Met" otherwise) — see README.
            destination_country=cfg.get("destination_country", ""),
        )
